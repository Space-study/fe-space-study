...
const Window: React.FC<{name: string; children: ReactNode}> = ({name, children}) => {
  ...
  return createPortal(
    <div
      ...
      <div
        className={`w-[90%] h-[90%] fixed top-1/2 left-1/2 bg-slate-500/70 shadow-slate-800 p-2 rounded-md transition-transform duration-500 ease-in-out ${isClosing ? 'transform scale-0 -translate-x-1/2 -translate-y-1/2' : 'transform scale-100 -translate-x-1/2 -translate-y-1/2'}`}>
