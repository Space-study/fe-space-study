    ...
    <Card className="overflow-hidden transition-all duration-300 hover:shadow-lg group">
      ...
      >
        <div className="relative">
          ...
          <Button
            variant="outline"
            size="icon"
            className={`absolute top-2 right-2 h-8 w-8 rounded-full shadow-md bg-background/90 backdrop-blur-sm border-none ${favorite ? "text-red-500 hover:text-red-600" : "text-muted-foreground hover:text-foreground"
              }`}
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <Badge variant="outline" className={`px-2.5 py-0.5 font-medium ${colorSet.bg} ${colorSet.text}`}>
            {data.category}
          </Badge>

          <div className="flex items-center gap-1.5 text-sm">
            <span
              className={`w-2 h-2 rounded-full ${data.status === "active" ? "bg-green-500 animate-pulse" : "bg-yellow-500"
                }`}
              aria-hidden="true"
            ></span>
