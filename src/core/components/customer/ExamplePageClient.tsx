...
import { setUser, clearUser, setAuthentication } from '@/core/redux/slices/userSlice';
  ...
  return (
    <div className='p-6 space-y-4'>
      ...
      <div>
        ...
        <p>Email: {email}</p>
        <p>Authenticated: {userFromStore.isAuthenticated ? 'Yes' : 'No'}</p> {/* Display authentication status */}
        <button className='mt-4 mr-2 bg-red-500 text-white py-2 px-4 rounded' onClick={() => dispatch(clearUser())}>Clear User</button>
        <button className='mt-4 mr-2 bg-red-500 text-white py-2 px-4 rounded' onClick={() => dispatch(setAuthentication(true))}>Set Authenticated</button> {/* Set user as authenticated */}
        <button className='mt-4 mr-2 bg-red-500 text-white py-2 px-4 rounded' onClick={() => dispatch(setAuthentication(false))}>Set Unauthenticated</button> {/* Set user as unauthenticated */}
      </div>
    </div>
  );
};
